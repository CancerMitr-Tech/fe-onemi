...
function MarqueeRow({ direction = "left" }: { direction?: "left" | "right" }) {
  ...
  return (
    <div className="overflow-hidden whitespace-nowrap py-2">
      ...
      `}</style>
      <div className={`inline-flex items-center ${direction === "left" ? "marquee-left" : "marquee-right"}`}>
        {items.map((item, i) => (
...
function StepCard({ step, index, containerScrollY }: { step: (typeof mhrSteps)[0]; index: number; containerScrollY: MotionValue<number> }) {
        ...
        <div className="sticky overflow-hidden" style={{ top: 80 }}>
          <div className="max-w-6xl mx-auto">
            <div className="relative overflow-hidden" style={{ height: CARD_H }}>
              {mhrSteps.map((step, i) => (
                <StepCard key={i} step={step} index={i} containerScrollY={containerScrollY} />
              ))}
            </div>
