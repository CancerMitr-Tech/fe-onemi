      ...
      <div className="bg-white rounded-2xl shadow-md p-8">
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 flex flex-col gap-4">
            <h3 className="text-xl font-bold text-[#1A1A2E]">What&apos;s included</h3>
            <ul className="space-y-3 flex-1">
              {PRICING_FEATURES.map((feature) => (
                <li key={feature} className="flex items-start gap-3">
                  <span className="flex-shrink-0 w-5 h-5 rounded-full bg-[#E85D04] flex items-center justify-center mt-0.5">
                    <svg
          ...
          <div className="lg:col-span-2 grid sm:grid-cols-3 gap-4">
            {PRICING_PLANS.map((plan) => (
              ...
              >
                {plan.badge && (
                  <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#E85D04] text-white text-xs font-bold px-3 py-1 rounded-full">
                    {plan.badge}
                  </span>
                )}
                <p className="font-semibold text-[#1A1A2E]">{plan.name}</p>
                <p className="text-2xl font-bold text-[#E85D04]">{plan.price}</p>
                <p className="text-xs text-[#6B7280]">{plan.note}</p>
              </div>
            ))}
          </div>
        </div>
