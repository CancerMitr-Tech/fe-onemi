  ...
  return (
    <div className="overflow-hidden whitespace-nowrap py-2">
      <style>{`
        ...
        .mdp-ml{animation:mdp-ml 70s linear infinite}
        .mdp-mr{animation:mdp-mr 70s linear infinite}
      `}</style>
      <div className={`inline-flex items-center ${direction === "left" ? "mdp-ml" : "mdp-mr"}`}>
        {repeated.map((item, i) => (
...
function MdpStepCard({ step, index, containerScrollY }: { step: (typeof mdpSteps)[0]; index: number; containerScrollY: MotionValue<number> }) {
        ...
        <div className="sticky overflow-hidden" style={{ top: 80 }}>
          <div className="max-w-6xl mx-auto">
            <div className="relative overflow-hidden" style={{ height: CARD_H }}>
              {mdpSteps.map((step, i) => (
                <MdpStepCard key={i} step={step} index={i} containerScrollY={containerScrollY} />
              ))}
            </div>
